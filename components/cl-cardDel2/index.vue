<template>
	<view v-if="cardcount>0" class="page" :style="{ height: `${sysHeight}px`, width: `${sysWidth}px` }">
        ...
        left: `${-sysWidth}px`
			...
			zIndex: `${999 - item._id}`, height: `${sysHeight}px`, width: `${sysWidth}px` }" direction="all" :x="item.moveX"
				:y="item.moveY" out-of-bounds v-if="index <= number" :disabled="index != 0" :animation="item.animation"
				@tap="tapCard(item)" @touchend="touchend" @touchmove="touchMove" @touchstart="touchStart">
              ...
              index < number
                ...
                : `${1 - (1 - opacity) * (number - 1)}`
					...
					<card-box :src="item.src" :number="item.number" :name="item.name" :sex="item.sex"
						:constellation="item.constellation" :address="item.address" :old="item.old"
						:latelyTimeStr="item.latelyTimeStr" ref="cardBox"></card-box>
				</view>
			</movable-view>
		</movable-area>
		<!-- #endif -->
		<!-- #ifdef APP-PLUS -->
		<view v-if="index <= number" class="move-view" :key="item._id" @touchend="touchend" @tap="tapCard(item)"
			v-for="(item, index) in dataList" @touchmove="touchMove" @touchstart="touchStart"
			:animation="animationData[index]" :style="{
		  height: `${sysHeight}px`, width: `${sysWidth-4}px`,
		  marginLeft:`${-sysWidth/2}px`,marginTop:`${-sysHeight/2}px`,
        transform:
          index < number
	...
	export default {
		...
		methods: {
				...
				Server.get("/user/getUserList", {
					...
					success: response => {
						...
						promise = new Promise((resolve, reject) => {
									...
									dataGroup.push({
										...
										address: self.getString(response.data.data.rows[i]
											...
											.replace("市", "") + (response.data.data.rows[i]
												...
												")" :
												""),
										name: self.getString(response.data.data.rows[i]
											.nickName),
										constellation: self.getString(response.data.data.rows[
												i]
											.xinzuo),
										number: self.getString(response.data.data.rows[i]
											.imgCount),
										old: self.getString(response.data.data.rows[i].age),
										id: self.getString(response.data.data.rows[i].id),
										latelyTimeStr: self.getString(response.data.data.rows[
												i]
											.latelyTimeStr)
									})
								}
							...
							self.dataList = [...self.dataList, ...dataGroup]
							resolve()
						})
			...
			delCard(x, y) {
				...
				this.currid=this.dataList[0].id;
				if (x > 0) {
			...
			toChat(userId, sendUserId) {
				...
				// });
				uni.navigateTo({
					url: "/pagesB/HM-chat/HM-chat?userId=" + userId + "&nickName=" + this.chatusername +
