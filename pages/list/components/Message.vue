		...
		<view class="message-list">
			<template v-if="messagelist && messagelist.length > 0">
				<view v-for="item in messagelist" :key="item.id"
					@tap="toChat(item.userId, item.nickName, item.sendUserId)" class="row message-item">
					...
					<view class="right">
						<view>
							...
							<view class="message">
								<!-- 文字消息 -->
								<view class="richtext"
									v-if="item.messageContentViewModel!=null&&item.messageContentViewModel.messageFormat!=null&&(item.messageContentViewModel.messageFormat=='text'||item.messageContentViewModel.messageFormat=='blog'||item.messageContentViewModel.messageFormat=='matching'||item.messageContentViewModel.messageFormat=='praise')">
									<rich-text :nodes="item.messageContentViewModel.messageContent"></rich-text>
								</view>
								<view class="richtext"
									v-else-if="item.messageContentViewModel!=null&&item.messageContentViewModel.messageFormat!=null&&item.messageContentViewModel.messageFormat=='emoji'">
									<rich-text :nodes="item.messageContentViewModel.times"></rich-text>
								</view>
								<view class="richtext"
									v-else-if="item.messageContentViewModel!=null&&item.messageContentViewModel.messageFormat!=null&&item.messageContentViewModel.messageFormat=='img'">
									图片
								</view>
								<view class="richtext"
									v-else-if="item.messageContentViewModel!=null&&item.messageContentViewModel.messageFormat!=null&&item.messageContentViewModel.messageFormat=='voice'">
									语音
								</view>
								<view class="richtext"
									v-else-if="item.messageContentViewModel!=null&&item.messageContentViewModel.messageFormat!=null&&item.messageContentViewModel.messageFormat=='game'">
									有人邀请了你的组局
								</view>
						...
						Server.get("/users/" + sendUserId, {}, {
							success: response => {
								...
								if (userinfo.onlySeeVip != null && userinfo.onlySeeVip == true) {
									//系统设置了仅会员可以聊天需要
									self.$emit("changmodel", true);
								} else {
									uni.navigateTo({
										url: "/pagesB/HM-chat/HM-chat?userId=" + userId + "&nickName=" +
											nickName +
											"&sendUserId=" +
											sendUserId,
									});
