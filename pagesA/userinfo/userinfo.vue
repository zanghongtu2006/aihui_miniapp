<template>
	<view class="page" :style="{ height: mineBoxHeight}">
		...
		<view v-if="isyn" class="header">
			<u-swiper @click="ViewImage" height="640" :list="imgs" :autoplay="false" :effect3d-previous-margin="100"
			...
			<view>
				...
				<view class="section">
					...
					<view class="contact-method content">
						...
						<text v-if="showqq" class="weixin">{{userinfo.qq}}</text>
						<text v-else class="weixin">
							<u-tag @click="seeqq" text="查看QQ" mode="light" shape="circle" bg-color="#EFFEF6"
								color="#72CD9D" border-color="#EFFEF6" />
						</text>
					</view>
				</view>
		...
		<view v-if="loginuserinfo!=null&&loginuserinfo.id!=userId" class="control">
			...
			<view
				v-if="(userinfo.matchingSuccess!=null&&userinfo.matchingSuccess==true)||userinfo.id==1||(userinfo.onlySeeVip!=null&&userinfo.onlySeeVip==false)"
				class="right" @click="toChat()">
